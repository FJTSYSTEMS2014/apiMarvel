...
const Get_Marvel = () => {
  ...
  useEffect (() => {
    fetch (
      'http://gateway.marvel.com/v1/public/characters?ts=1&apikey=344d40df0c8cc373141c1dc321fae9cf&hash=bd0722d5750b6362d5ba0212ca36726b'
    ) // hacemos la petición get
      .then (res => res.json ()) // cuando hayamos terminado (then) e texto plano lo parseamos a json la respuesta de la petición
      .then (res => setName (res.data.results[0].comics.items[0].name)); // cuando hayamos terminado (then) actualizamos el estado nombre
